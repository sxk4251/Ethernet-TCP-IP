...
uint8_t etherIsTcp(uint8_t data[])
{
    ...
    if (ok)
    {
        ...
        tmp_int = ip->protocol;
        sum += (tmp_int & 0xff) << 8;
       // etherSumWords(&tcp->dataOffset, 1);
		tmp_int=ntohs(ip->length);
		x=((ip->rev_size & 0xF)*4);
		y=((tcp->dataOffset & 0xF0)>>4)*4;
		sum+=((y&0xff)<<8);
		tmp_int-=x;
        tmp_int-=y;
        sum += ((tmp_int&0xff)<<8);
...
// Gets point8_ter to TCP payload of frame
